...
import { createCardTransaction, createCardTransactions, getCardTransaction, getCardTransactions, newIdempotencyKey } from "./cardTransactions";
...
describe("card transaction API boundary", () => {
  ...
  it("serializes POST money as an exact string and validates its response", async () => {
    ...
    const result = await createCardTransaction({ description: "Test", amount: { amount: 9223372036854775807n, currency: "USD" }, type: "CREDIT" }, "key-1");
    ...
    await expect(createCardTransaction({ description: "Test", amount: { amount: 125n, currency: "USD" }, type: "CREDIT" }, "key-2")).rejects.toBeInstanceOf(ApiContractError);
  });
  ...
  it("describes a request that violates the contract without dumping the request body", async () => {
    ...
    for (const call of [() => createCardTransaction(input, "key-1"), () => createCardTransactions([input], "key-1")]) {
  ...
  it("posts an array for a batch and maps every returned item", async () => {
    ...
    const result = await createCardTransactions(inputs, "key-batch");
  ...
  it("refuses to send an empty batch and rejects a batch response that violates the contract", async () => {
    ...
    await expect(createCardTransactions([], "key-1")).rejects.toThrow(/violates the API contract/);
    expect(fetch).not.toHaveBeenCalled();
    respond(cardTransaction, 201); // a bare object is not the list shape
    await expect(createCardTransactions([{ description: "Test", amount: { amount: 1n, currency: "USD" }, type: "DEBIT" }], "key-1")).rejects.toBeInstanceOf(ApiContractError);
  });
  ...
  it("sends the Idempotency-Key it was given on single and batch creates", async () => {
    const input = { description: "Test", amount: { amount: 1n, currency: "USD" as const }, type: "DEBIT" as const };
    let fetch = respond(cardTransaction, 201);
    await createCardTransaction(input, "key-1");
    expect(fetch.mock.calls[0][1].headers).toEqual({ Accept: "application/json", "Content-Type": "application/json", "Idempotency-Key": "key-1" });

    fetch = respond({ items: [cardTransaction] }, 201);
    await createCardTransactions([input], "key-2");
    expect(fetch.mock.calls[0][1].headers).toEqual({ Accept: "application/json", "Content-Type": "application/json", "Idempotency-Key": "key-2" });
  });

  it("maps an idempotency conflict to an ApiError the page can show", async () => {
    respond({ code: "IDEMPOTENCY_CONFLICT", message: "Idempotency-Key was already used with a different request" }, 422);
    const error = await createCardTransactions([{ description: "Test", amount: { amount: 1n, currency: "USD" }, type: "DEBIT" }], "key-1").catch((e: unknown) => e);
    expect(error).toMatchObject({ name: "ApiError", status: 422, code: "IDEMPOTENCY_CONFLICT" });
    expect(describeError(error)).toBe("Request failed (422 IDEMPOTENCY_CONFLICT): Idempotency-Key was already used with a different request");
  });

  it("mints a distinct UUID key per logical submission", () => {
