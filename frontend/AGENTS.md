...
- The validators are precompiled: `scripts/generate-validators.mjs` turns `contracts/schemas/*.schema.json` into `src/api/validators.generated.{js,d.ts}` (checked in, regenerated by `npm run generate:validators`, `pretest` and `prebuild`; never hand-edited), and `schemas.ts` re-exports them with their wire types. Do not instantiate Ajv or call `ajv.compile` in app code: the production build must run under `Content-Security-Policy: script-src 'self'` (no `unsafe-eval`), which `npm run preview` enforces and the tests check: `vitest.config.ts` starts every test worker with `--disallow-code-generation-from-strings` (`test.execArgv`), so `npm test` fails on a validator that went back to runtime compilation.
...
- Tests run with `npm test -- --run`; `npm run build` type-checks and builds for production. Both regenerate the validators first.
