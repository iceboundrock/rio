...
- Pages handle the loading, error, contract-error (`ApiContractError` for responses, `RequestContractError` for outgoing bodies), and empty states that apply to their operation. `src/api/client.ts` wraps a rejected `fetch` in `NetworkError`; that is the only signal `describeError` treats as an unreachable server, so never classify a bare `TypeError` as a network failure.
